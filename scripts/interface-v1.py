                    ...
                    if re.match(r'\bcs[a-z0-9]{5}\b', qr_data) or re.match(r'^[A-Za-z0-9]{7}$', qr_data):
                        ...
                        if confirmacion in ["sí", "si", "Si", "Sí"]:
                            if verificar_existencia_en_excel(serial_number):
                                ...
                                asset_data = procesar_qr_laptop("Dell", serial_number)
                                agregar_a_excel(asset_data)
                        else: 
                            messagebox.showinfo("Información", "Operación cancelada por el usuario.")
                            return
                    ...
                    if re.match(r'\bC02[A-Za-z0-9]{8,10}\b', qr_data) or re.match(r'^[A-Za-z0-9]{10,12}$', qr_data) or re.match(r'^S?[C02][A-Za-z0-9]{8}$', qr_data) or re.match(r'^S[A-Za-z0-9]{9}$', qr_data):
                        ...
                        if confirmacion in ["sí", "si", "Si", "Sí"]:
                            if verificar_existencia_en_excel(serial_number):
                                ...
                                asset_data = procesar_qr_laptop("Mac", serial_number)
                                agregar_a_excel(asset_data)
                        else: 
                            messagebox.showinfo("Información", "Operación cancelada por el usuario.")
                            return
...
def entregar_laptop():
    try: 
        ...
        serial_number, manufacturer = manejar_qr_laptop("Deliver")
    ...
    while True:
        ...
        while True:
            ...
            for qr in qr_codes:
                ...
                flag = es_codigo_valido(qr_data)
                if flag == "dell":
                    messagebox.showinfo("Información", f"Código QR dell escaneado: \n{qr_data}")
                    cap.release()
                    cv2.destroyAllWindows()
                    return qr_data
                elif flag == "mac":
                    messagebox.showinfo("Información", f"Código QR mac escaneado: \n{qr_data}")
                    cap.release()
                    cv2.destroyAllWindows()
                    return qr_data
                elif flag == "monitor":
                    messagebox.showinfo("Información", f"Código QR monitor escaneado: \n{qr_data}")
                    cap.release()
                    cv2.destroyAllWindows()
                    return qr_data
                elif flag == "invalido":
...
tk.Button(frame, text="Escanear QR y registrar laptop (Dell/Mac)", command=lambda: manejar_qr_laptop("Register")).pack()
tk.Button(frame, text="Entregar laptop a un usuario", command=entregar_laptop).pack()
tk.Button(frame, text="Escanear QR y registrar en Excel (Template Default)", command=lambda: agregar_a_excel(parse_qr_data_template(escanear_qr_con_celular()))).pack()
